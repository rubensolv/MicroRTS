...
graph = tf.Graph()
builder = tf.saved_model.builder.SavedModelBuilder('./mod1')
...
cmd1= tf.nn.relu(full_layer(flatten, largura*altura*2))


full_2 = tf.nn.relu(full_layer(cmd1, largura*altura*grupos))
...
gd_step = tf.train.GradientDescentOptimizer(1.0).minimize(cross_entropy,name="treina")
