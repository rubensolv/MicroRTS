 ...
 * @author rubens
 */
public class A3NWithinGeneratorNoWait implements ICMAB_ActionGenerator {
    ...
    public A3NWithinGeneratorNoWait(GameState gs_to_start_from, int playerForThisComputation, UnitTypeTable a_utt, String Behavior, int qtdUNits, List<AI> abstraction) throws Exception {
    ...
    private List<UnitAction> removeWaitIfExists(List<UnitAction> l) {
        List<UnitAction> ret = new ArrayList<>();
        for (UnitAction uact : l) {
            if (uact.getType() == 0) {                
                ret.add(uact);
            }
        }
        ...
        for (Unit u : gs_to_start_from.getUnits()) {
            if (u.getPlayer() == playerForThisComputation && !unitsControled.contains(u)) {
                if (gs_to_start_from.getUnitActions().get(u) == null) {
                    ...
                    if (l.size() > 0) {
                        //remove wait if l.size > 1                        
                        if (l.size() > 1) {
                            //check if there is wait
                            l = removeWaitIfExists(l);
                        }
                    } else {
