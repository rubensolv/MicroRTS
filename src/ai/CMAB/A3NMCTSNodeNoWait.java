 ...
 * @author santi
 */
public class A3NMCTSNodeNoWait extends MCTSNode {
    ...
    HashMap<BigInteger, A3NMCTSNodeNoWait> childrenMap = new LinkedHashMap<>();    // associates action codes with children
    ...
    public A3NMCTSNodeNoWait(int maxplayer, int minplayer, GameState a_gs, A3NMCTSNodeNoWait a_parent, 
        ...
        if (gs.winner() != -1 || gs.gameover()) {
            ...
            type = 0;
            moveGenerator = new A3NNoWaitGenerator(gs,maxplayer, utt, behavior, qtdUnits, abstraction);
            ...
            type = 1;
            moveGenerator = new A3NNoWaitGenerator(gs,minplayer, utt, behavior, qtdUnits, abstraction);
            actions = new ArrayList<>();
            children = new ArrayList<>();
            unitActionTable = new LinkedList<>();
            multipliers = new BigInteger[moveGenerator.getChoices().size()];
    ...
    // Alternated Sampling:
    public A3NMCTSNodeNoWait selectLeaf(int maxplayer, int minplayer, float epsilon_l, float epsilon_g, float epsilon_0, int global_strategy, int max_depth, int a_creation_ID) throws Exception {
        ...
        if (children.size() > 0 && r.nextFloat() >= epsilon_0) {
            ...
            A3NMCTSNodeNoWait selected = null;
    ...
    public A3NMCTSNodeNoWait selectFromAlreadySampledEpsilonGreedy(float epsilon_g) throws Exception {
        if (r.nextFloat() >= epsilon_g) {
            ...
            A3NMCTSNodeNoWait best = (A3NMCTSNodeNoWait) children.get(r.nextInt(children.size()));
            return best;
        }
    }

    public A3NMCTSNodeNoWait selectFromAlreadySampledUCB1(float C) throws Exception {
        A3NMCTSNodeNoWait best = null;
        double bestScore = 0;
        for (MCTSNode pate : children) {
    ...
    public A3NMCTSNodeNoWait selectLeafUsingLocalMABs(int maxplayer, int minplayer, float epsilon_l, float epsilon_g, float epsilon_0, int global_strategy, int max_depth, int a_creation_ID) throws Exception {
        PlayerAction pa2;
        ...
        A3NMCTSNodeNoWait pate = childrenMap.get(actionCode);
        if (pate == null) {
            actions.add(pa2);
            GameState gs2 = gs.cloneIssue(pa2);
            A3NMCTSNodeNoWait node = new A3NMCTSNodeNoWait(maxplayer, minplayer, gs2.clone(), this, evaluation_bound, a_creation_ID, forceExplorationOfNonSampledActions, utt, behavior, qtdUnits, abstraction);
    ...
    public void propagateEvaluation(double evaluation, A3NMCTSNodeNoWait child) {
...
//        if (child!=null) System.out.println(evaluation);
        ...
        if (parent != null) {
            ((A3NMCTSNodeNoWait) parent).propagateEvaluation(evaluation, this);
        }
    }
    ...
    private A3NMCTSNodeNoWait selectFromAlreadySampledAlternate() {
