 ...
 * @author santi
 */
public class A3NWithinMCTSNodeNoWait extends MCTSNode {
    ...
    HashMap<BigInteger, A3NWithinMCTSNodeNoWait> childrenMap = new LinkedHashMap<>();    // associates action codes with children
    ...
    public A3NWithinMCTSNodeNoWait(int maxplayer, int minplayer, GameState a_gs, A3NWithinMCTSNodeNoWait a_parent, 
        ...
        if (gs.winner() != -1 || gs.gameover()) {
            ...
            type = 0;
            moveGenerator = new A3NWithinGeneratorNoWait(gs,maxplayer, utt, behavior, qtdUnits, abstraction);
            ...
            type = 1;
            moveGenerator = new A3NWithinGeneratorNoWait(gs,minplayer, utt, behavior, qtdUnits, abstraction);
            actions = new ArrayList<>();
            children = new ArrayList<>();
            unitActionTable = new LinkedList<>();
            multipliers = new BigInteger[moveGenerator.getChoices().size()];
    ...
    // Alternated Sampling:
    public A3NWithinMCTSNodeNoWait selectLeaf(int maxplayer, int minplayer, float epsilon_l, float epsilon_g, float epsilon_0, int global_strategy, int max_depth, int a_creation_ID) throws Exception {
        ...
        if (children.size() > 0 && r.nextFloat() >= epsilon_0) {
            ...
            A3NWithinMCTSNodeNoWait selected = null;
    ...
    public A3NWithinMCTSNodeNoWait selectFromAlreadySampledEpsilonGreedy(float epsilon_g) throws Exception {
        if (r.nextFloat() >= epsilon_g) {
            ...
            for (MCTSNode pate : children) {
                if (type == 0) {
                    ...
                    if (best == null || (pate.accum_evaluation / pate.visit_count) > (best.accum_evaluation / best.visit_count)) {
                        best = (A3NWithinMCTSNodeNoWait) pate;
                    }
                } else {
                    // min node:
                    if (best == null || (pate.accum_evaluation / pate.visit_count) < (best.accum_evaluation / best.visit_count)) {
            ...
            A3NWithinMCTSNodeNoWait best = (A3NWithinMCTSNodeNoWait) children.get(r.nextInt(children.size()));
            return best;
        }
    }

    public A3NWithinMCTSNodeNoWait selectFromAlreadySampledUCB1(float C) throws Exception {
        A3NWithinMCTSNodeNoWait best = null;
        double bestScore = 0;
        for (MCTSNode pate : children) {
    ...
    public A3NWithinMCTSNodeNoWait selectLeafUsingLocalMABs(int maxplayer, int minplayer, float epsilon_l, float epsilon_g, float epsilon_0, int global_strategy, int max_depth, int a_creation_ID) throws Exception {
        PlayerAction pa2;
        ...
        A3NWithinMCTSNodeNoWait pate = childrenMap.get(actionCode);
        if (pate == null) {
            actions.add(pa2);
            GameState gs2 = gs.cloneIssue(pa2);
            A3NWithinMCTSNodeNoWait node = new A3NWithinMCTSNodeNoWait(maxplayer, minplayer, gs2.clone(), this, evaluation_bound, a_creation_ID, forceExplorationOfNonSampledActions, utt, behavior, qtdUnits, abstraction);
    ...
    public void propagateEvaluation(double evaluation, A3NWithinMCTSNodeNoWait child) {
...
//        if (child!=null) System.out.println(evaluation);
        ...
        if (parent != null) {
            ((A3NWithinMCTSNodeNoWait) parent).propagateEvaluation(evaluation, this);
        }
    }
    ...
    private A3NWithinMCTSNodeNoWait selectFromAlreadySampledAlternate() {
