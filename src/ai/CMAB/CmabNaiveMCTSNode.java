 ...
 * @author santi
 */
public class CmabNaiveMCTSNode extends MCTSNode {
    ...
    HashMap<BigInteger, CmabNaiveMCTSNode> childrenMap = new LinkedHashMap<BigInteger, CmabNaiveMCTSNode>();    // associates action codes with children
    // Decomposition of the player actions in unit actions, and their contributions:
    public List<CmabNaiveUnitActionTableEntry> unitActionTable = null;
    ...
    public CmabNaiveMCTSNode(int maxplayer, int minplayer, GameState a_gs, CmabNaiveMCTSNode a_parent, double a_evaluation_bound, int a_creation_ID, boolean fensa, UnitTypeTable a_utt, String classGenerator) throws Exception {
        ...
        if (gs.winner() != -1 || gs.gameover()) {
            ...
            unitActionTable = new LinkedList<CmabNaiveUnitActionTableEntry>();
            ...
            for (Pair<Unit, List<UnitAction>> choice : moveGenerator.getChoices()) {
                CmabNaiveUnitActionTableEntry ae = new CmabNaiveUnitActionTableEntry();
                ae.u = choice.m_a;
    ...
    // Naive Sampling:
    public CmabNaiveMCTSNode selectLeaf(int maxplayer, int minplayer, float epsilon_l, float epsilon_g, float epsilon_0, int global_strategy, int max_depth, int a_creation_ID) throws Exception {
        ...
        if (children.size() > 0 && r.nextFloat() >= epsilon_0) {
            ...
            CmabNaiveMCTSNode selected = null;
    ...
    public CmabNaiveMCTSNode selectFromAlreadySampledEpsilonGreedy(float epsilon_g) throws Exception {
        if (r.nextFloat() >= epsilon_g) {
            ...
            CmabNaiveMCTSNode best = (CmabNaiveMCTSNode) children.get(r.nextInt(children.size()));
            return best;
        }
    }

    public CmabNaiveMCTSNode selectFromAlreadySampledUCB1(float C) throws Exception {
        CmabNaiveMCTSNode best = null;
        double bestScore = 0;
        for (MCTSNode pate : children) {
    ...
    public CmabNaiveMCTSNode selectLeafUsingLocalMABs(int maxplayer, int minplayer, float epsilon_l, float epsilon_g, float epsilon_0, int global_strategy, int max_depth, int a_creation_ID) throws Exception {
        PlayerAction pa2;
        ...
        for (CmabNaiveUnitActionTableEntry ate : unitActionTable) {
        ...
        while (!notSampledYet.isEmpty()) {
            ...
            try {
                CmabNaiveUnitActionTableEntry ate = unitActionTable.get(i);
                int code;
                UnitAction ua;
                ResourceUsage r2;
        ...
        CmabNaiveMCTSNode pate = childrenMap.get(actionCode);
        if (pate == null) {
            actions.add(pa2);
    ...
    public CmabNaiveUnitActionTableEntry getActionTableEntry(Unit u) {
        for (CmabNaiveUnitActionTableEntry e : unitActionTable) {
            if (e.u == u) {
                return e;
            }
    ...
    public void propagateEvaluation(double evaluation, CmabNaiveMCTSNode child) {
...
//        if (child!=null) System.out.println(evaluation);
        ...
        if (child != null) {
            ...
            for (Pair<Unit, UnitAction> ua : pa.getActions()) {
                CmabNaiveUnitActionTableEntry actionTable = getActionTableEntry(ua.m_a);
        ...
        if (parent != null) {
            ((CmabNaiveMCTSNode) parent).propagateEvaluation(evaluation, this);
        }
    }

    public void printUnitActionTable() {
        for (CmabNaiveUnitActionTableEntry uat : unitActionTable) {
