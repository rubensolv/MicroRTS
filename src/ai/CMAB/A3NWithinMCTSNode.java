 ...
 * @author santi
 */
public class A3NWithinMCTSNode extends MCTSNode {
    ...
    HashMap<BigInteger, A3NWithinMCTSNode> childrenMap = new LinkedHashMap<>();    // associates action codes with children
    ...
    public A3NWithinMCTSNode(int maxplayer, int minplayer, GameState a_gs, A3NWithinMCTSNode a_parent, 
    ...
    // Alternated Sampling:
    public A3NWithinMCTSNode selectLeaf(int maxplayer, int minplayer, float epsilon_l, float epsilon_g, float epsilon_0, int global_strategy, int max_depth, int a_creation_ID) throws Exception {
        ...
        if (children.size() > 0 && r.nextFloat() >= epsilon_0) {
            ...
            A3NWithinMCTSNode selected = null;
    ...
    public A3NWithinMCTSNode selectFromAlreadySampledEpsilonGreedy(float epsilon_g) throws Exception {
        if (r.nextFloat() >= epsilon_g) {
            ...
            A3NWithinMCTSNode best = (A3NWithinMCTSNode) children.get(r.nextInt(children.size()));
            return best;
        }
    }

    public A3NWithinMCTSNode selectFromAlreadySampledUCB1(float C) throws Exception {
        A3NWithinMCTSNode best = null;
        double bestScore = 0;
        for (MCTSNode pate : children) {
    ...
    public A3NWithinMCTSNode selectLeafUsingLocalMABs(int maxplayer, int minplayer, float epsilon_l, float epsilon_g, float epsilon_0, int global_strategy, int max_depth, int a_creation_ID) throws Exception {
        PlayerAction pa2;
        ...
        A3NWithinMCTSNode pate = childrenMap.get(actionCode);
        if (pate == null) {
            actions.add(pa2);
            GameState gs2 = gs.cloneIssue(pa2);
            A3NWithinMCTSNode node = new A3NWithinMCTSNode(maxplayer, minplayer, gs2.clone(), this, evaluation_bound, a_creation_ID, forceExplorationOfNonSampledActions, utt, behavior, qtdUnits, abstraction);
    ...
    public void propagateEvaluation(double evaluation, A3NWithinMCTSNode child) {
...
//        if (child!=null) System.out.println(evaluation);
        ...
        if (parent != null) {
            ((A3NWithinMCTSNode) parent).propagateEvaluation(evaluation, this);
        }
    }
    ...
    private A3NWithinMCTSNode selectFromAlreadySampledAlternate() {
