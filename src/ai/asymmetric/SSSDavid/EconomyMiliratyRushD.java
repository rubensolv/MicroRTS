 ...
 * @author rubensolv
 */
public class EconomyMiliratyRushD extends AbstractionLayerAI {
    ...
    public EconomyMiliratyRushD(UnitTypeTable a_utt) {
        this(a_utt, new AStarPathFinding());
    }

    public EconomyMiliratyRushD(UnitTypeTable a_utt, PathFinding a_pf) {
        super(a_pf);
        reset(a_utt);
    }
    ...
    @Override
    public AI clone() {
        return new EconomyMiliratyRushD(utt, pf);
    }
    ...
    public void workersBehavior(List<Unit> workers, Player p,  GameState gs,Information inf){
        ...
        PhysicalGameState pgs = gs.getPhysicalGameState();
       if(inf.contruindo_barraca)nbarracks = 1;
        ...
        for (Unit u2 : pgs.getUnits()) {
            ...
            else  if ( (u2.getType() == lightType || u2.getType() == rangedType || u2.getType() == heavyType)
                    && u2.getPlayer() == p.getID()) {
                nArmyUnits++;
            }
        }
        ...
        if (nbarracks == 0 && !freeWorkers.isEmpty()) {
            ...
            if (p.getResources() >= barracksType.cost + resourcesUsed) {
                ...
                inf.contruindo_barraca = true;
                inf.barraca_tempo = gs.getTime();
            }
