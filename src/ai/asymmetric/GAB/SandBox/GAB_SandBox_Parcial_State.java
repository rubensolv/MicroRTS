...
/**
 *
 * @author rubens Esta implementação remove as unidades inimigas que não estão
 * participando diretamente do confronto.
 */
public class GAB_SandBox_Parcial_State extends AIWithComputationBudget implements InterruptibleAI {
    ...
    public GAB_SandBox_Parcial_State(UnitTypeTable utt) {
    ...
    public GAB_SandBox_Parcial_State(UnitTypeTable utt, int numUnits, int numManager) {
        this(100, 150, new SimpleSqrtEvaluationFunction3(), utt, new AStarPathFinding(), numUnits, numManager);
    }

    public GAB_SandBox_Parcial_State(int time, int max_playouts, EvaluationFunction e, UnitTypeTable a_utt, PathFinding a_pf) {
    ...
    public GAB_SandBox_Parcial_State(int time, int max_playouts, EvaluationFunction e, UnitTypeTable a_utt, PathFinding a_pf, int numUnits, int numManager) {
        super(time, max_playouts);
    ...
    @Override
    public AI clone() {
        return new GAB_SandBox_Parcial_State(_time, _max_playouts, evaluation, utt, pf);
    }
    ...
    public PlayerAction getBestActionSoFar() throws Exception {
        ...
        if ((System.currentTimeMillis() - start) < 90) {
            ...
            //definir novo cenário para o estado a ser usado no AB
            GameState gs_AB = defineNewState();            
            
            PlayerAction paAB = _ab.getActionForAssymetric(playerForThisComputation, gs_AB, currentScriptData, _unitsAbsAB);
    ...
    public String toString() {
        //return "GAB_SandBox_Parcial_State{" + "_numUnits=" + _numUnits + ", numManager=" + _numManager + '}';
        return "GAB_SandBox_Parcial_State" + _numUnits + "_" + _numManager;
    }

    private GameState defineNewState() {
        GameState gsTemp = this.gs_to_start_from.clone();

        //remove as unidades Inimigas distantes das nossas unidades controladas.
        ArrayList<Unit> UnitEnemy = getUnitsEnemyToRemover(_unitsAbsAB, gsTemp, _numUnits);
