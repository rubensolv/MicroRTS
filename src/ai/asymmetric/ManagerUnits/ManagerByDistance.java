 ...
 * @author rubens
 */
public class ManagerByDistance extends ManagerAbstraction {    
    double minDistance = 4.0;
    public ManagerByDistance(int _playerID, int numUnits) {
    ...
    private long getIDUnitAdd(GameState state, HashSet<Unit> unidades) {        
        for (Unit t : ManagerAbstraction.getUnitsPlayer(state, getPlayerID())) {
            if (!unitExistInArray(t, unidades)) {                
                //get the clossest enemy
                Unit enemy = getClosestEnemyUnit(t, state);                
                if(getDistanceSqToUnit(t.getX(), t.getY(), enemy.getX(), enemy.getY()) <= (this.minDistance+t.getAttackRange())){
                    return t.getID();
                }
            }
        }                
