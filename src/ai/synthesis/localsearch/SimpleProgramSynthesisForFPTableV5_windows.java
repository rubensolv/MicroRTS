 ...
 * @author rubens
 */
public class SimpleProgramSynthesisForFPTableV5_windows implements LocalSearch {

    //private final String pathTableScripts = System.getProperty("user.dir").concat("/Table/");
    //private ScriptsTable scrTable;
    private SAForFPTableV5_windows searchAlgorithm;
    private BuilderDSLTreeSingleton builder;
    private String uniqueID = UUID.randomUUID().toString();
    private HashMap<String, Fp_element> fp_group;
    private Queue<String> window;

    public SimpleProgramSynthesisForFPTableV5_windows(SAForFPTableV5_windows search) {
        ...
        this.window = new CircularFifoQueue<>(SettingsAlphaDSL.get_windows_size());
    }

    public SimpleProgramSynthesisForFPTableV5_windows(SAForFPTableV5_windows search, List<iDSL> initial_fp_group) {
        this(search);
    ...
    private void check_inclusion(iDSL idsl) {
        ...
        if (this.fp_group.containsKey(elem)) {
            this.fp_group.get(elem).incrementCount(); //with normal setting
            //this.fp_group.get(elem).setCounter(1); //with weight as 1 for any element
        } else {
            Fp_element newElem = new Fp_element(idsl);
            this.fp_group.put(elem, newElem);
            this.window.add(elem);
            if(fp_group.keySet().size() > window.size()){
                //clean fp_group
                ArrayList<String> remove_keys = new ArrayList<>();
                for (String key : fp_group.keySet()) {
                    if(!window.contains(key)){
                        remove_keys.add(key);
                    }                    
                }
                for (String remove_key : remove_keys) {
                    fp_group.remove(remove_key);
                }
            }
        }
    }
