...
public class FPTableV1Elo implements LocalSearch {
    ...
    private void check_inclusion(iDSL idsl) {
        ...
        if(this.cum_ind.containsKey(elem)){
            ...
            this.cum_ind.get(elem).setCounter(1);
        }else{
            Fp_element newElem = new Fp_element(idsl);
            this.cum_ind.put(elem, newElem);
        }
        
        if(this.cum_ind.size() > this.MAX_FP_GROUP_SIZE){
            perform_elo_eval();
        }
    }

    private void perform_elo_eval() {
